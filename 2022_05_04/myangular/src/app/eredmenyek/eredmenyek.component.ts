...
@Component({
  ...
  styleUrls: ['./eredmenyek.component.css']
})
export class EredmenyekComponent implements OnInit {
  ...
  objektumFeltolto(feltoltendoElem:string[]):Esemeny[]{
    const beolvasottAdatok:Esemeny[]=[];
    ...
    for (let i = 0; i < feltoltendoElem.length; i++) {
      ...
      beolvasottAdatok.push(esemenyElem);

    }return beolvasottAdatok;
  }
  esemenyAdatok=this.objektumFeltolto(this.eredmenyek);
}
...
export class Esemeny implements Sport {
  helyezes: number;
  csapatMeret: number;
  sportAg: string;
  versenySzam: string;
